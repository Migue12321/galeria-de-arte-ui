...
import ModalEdit from "../Modal/EditModal.js";
import {
  createMuiTheme,
  MuiThemeProvider,
  makeStyles,
...
export default function Gallery(props) {
  ...
  const deleteImage = (i) => {
    let url = process.env.REACT_APP_BACKEND_API + "image/" + props.paints[i]._id;
    fetch(url, {
      method: "DELETE",
      credentials: "same-origin",
      headers: { "Content-Type": "application/json; charset=UTF-8" },
      ...
      .then((res) => {
        if (res.status >= 200 || res.status > 400) {
          let imageRef = firebase
            .storage()
            .refFromURL(`gs://galleria-de-arte.appspot.com/pictures/${title}`);
          imageRef
            .delete()
            .then(function () {
              window.location.reload();
                      ...
                      <Grid container spacing={4}>
                        ...
                        <Grid item xs sm>
                          <Button
                            size="small"
                            color="primary"
                            onClick={handleOpenEdit.bind(this, card)}
                          >
                            Editar
                          </Button>
                        </Grid>
                        <Grid item xs sm>
                          <Button
                            size="small"
                            color="secondary"
                            onClick={deleteImage.bind(this, card)}
                          >
                            Eliminar
                          </Button>
                        </Grid>
